...
<div class="card">
    ...
    <div class="card-body" style="padding:30px;">
        ...
        <table width="100%" id="table_tipovariable" class="table table-striped table-bordered nowrap" >
                <thead>
                    <tr>
                         <th>Id</th>
                         <th>Nombre</th>
                 ...
                 <tbody>
                    ...
                    <tr>
                       <td>{{$item->id}}</td>
                       <td>{{$item->name_tipo_variables}}</td>
                       <td>{{$item->sigla_tipo_variables}}</td>
                       <td>{{$item->description_tipo_variables}}</td>
                       <td>
                        <div class="form-inline">
                            ...
                            <div class="form-group">
                                <form class="mb-2" role="form"  method="POST" action="{{ url('/deletetipovariable') }}"> 
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Tipo Variables</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form  role="form" method="POST" action="{{ url('/tipovariables') }}">
...
<script type="text/javascript">
    $(document).ready(function($){
      ...
      $(".li_tipovariables").addClass('active');


        //  $('#dgdfg').click();
        var Table = $('#table_tipovariable').DataTable({
          'bFilter': true,
          'bInfo': true,
          'responsive': true,
          "bAutoWidth": true,
    ...
    $(".btn_edit").click(function(){
            $("#content_sigla").slideDown();
             var data = Table.row( $(this).parents('tr') ).data();
              $("input[name=name]").val(data[1]);
                /// $("input[name=name]").trigger("click");
              $("input[name=id]").val(data[0]);
              $("input[name=sigla]").val(data[2]);
              $("textarea[name=description]").val(data[3]);
