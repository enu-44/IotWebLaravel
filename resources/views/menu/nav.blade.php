<header id="header" class="media">
            <div class="pull-left h-logo">
                <a href="{{ url('/') }}" class="hidden-xs">
                     {{ config('app.name') }}
                    <small style="margin-top: 10px">{{ __('Technology integration') }}</small>
                </a>
            ...
            <ul class="pull-right h-menu ">
                ...
                <li class="dropdown hm-profile">
                    <a data-toggle="dropdown" href="{{ url('/') }}">
                        
                        @if(Auth::user()->path===''  || Auth::user()->path===NULL)
                             <img src="/img/profile-pics/1.jpg" alt="">
                            @else
                            <img  src="http://localhost:8000/{{ Auth::user()->path }}" alt="">
                            @endif
                    </a>
                    ...
                    <ul class="dropdown-menu pull-right dm-icon">
                        ...
                        <li>
                            <a href="{{ route('logout') }}" onclick="event.preventDefault();
                                                     document.getElementById('logout-form').submit();"><i class="zmdi zmdi-time-restore"></i> {{ __('Logout') }}</a>
